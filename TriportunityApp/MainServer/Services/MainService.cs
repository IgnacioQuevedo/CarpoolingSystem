...
namespace MainServer.Services
{
    public class MainService : AdministrativeService.AdministrativeServiceBase
    {
        ...
        public override Task<RideResponse> AddRide(RideRequest request, ServerCallContext context)
        {
            try
            {
                var ride = ConvertToDomainRide(request);
                _rideRepository.CreateRide(ride);
                return Task.FromResult(new RideResponse { Status = "Ride added successfully" });
            }
            catch (Exception ex)
            {
